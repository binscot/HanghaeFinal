...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        //String userName = accessor.getFirstNativeHeader("Authorization").substring(7);
        // 토큰의 값만 확인 (로그인 여부를 확인하기 위함) - 헤더의 토큰값을 빼오기
        log.info("~~~~~~~~~ message : "+message);
        log.info("~~~~~~~~~ accessor : " + accessor);
        // 프론트에서는 유저네임을 보낼거다 토큰말고
        // 로그인 안한 유저면 NULL로 보내거나 해라 ㅇㅇㅇ
       ...
       log.info("~~~~~~~~ accessor.getLogin : " + accessor.getLogin());
        ...
        log.info("~~~~~~~~ Web Socket 들어올 때 userName 검증 = {}", userName);
        // websocket 연결시 헤더의 jwt userName 검증 ( 만약 CONNECT라면 -> 초기 연결임 )
        ...
        if (StompCommand.CONNECT == accessor.getCommand()) {
            ...
            //
            //String username = jwtDecoder.decodeUsername(userName); // decodeUsername에서 이미 토큰 validation 검사를 한다.
...
//            }
        ...
        // 구독 했는지 확인
            ...
            if (postId != null) {
//                String username = jwtDecoder.decodeUsername(userName);
