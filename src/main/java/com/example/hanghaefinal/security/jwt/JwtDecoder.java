...
import static com.example.hanghaefinal.security.jwt.JwtTokenUtils.*;
...
public class JwtDecoder {
    ...
    public String decodeUserId(String token) {
        DecodedJWT decodedJWT = isValidToken(token)
                .orElseThrow(() -> new IllegalArgumentException("유효한 토큰이 아닙니다.3"));
        ...
        Date now = new Date();
        if (expiredDate.before(now)) {
            throw new IllegalArgumentException("유효한 토큰이 아닙니다.4");
        }
